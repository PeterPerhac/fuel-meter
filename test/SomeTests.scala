...
class SomeTests extends FlatSpec with Matchers with Inside with Inspectors {
  ...
  "Double field validation" should "reject values below min bound" in {
    forAll(List(0.9999, 0.0, -1.23))(d => inside(constraint(d)) { case Invalid(errors) => errors.head.message shouldBe "error.range.below" })
  }

  "Double field validation" should "reject values above max bound" in {
    forAll(List(10.001, 12.34, 10000))(d => inside(constraint(d)) { case Invalid(errors) => errors.head.message shouldBe "error.range.above" })
  }
  ...
  "Double field validation constraint" should "complain if min >= max" in {
    assertThrows[AssertionError](inRange(2.0, 1.0).apply(5.0))
  }
  ...
  "Optional String field validation" should "accept string matching pattern" in {
    forAll(List("2016/12/17", "2015/04/10"))(stringConstraint(_) shouldBe Valid)
  }
